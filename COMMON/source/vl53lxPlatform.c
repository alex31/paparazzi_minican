 ...
 * supplied by the dynamically allocated IMAV sensor object.
 */
...
{
  return imav_vl53l4cx_work_buffer((const void *)dev, required_size);
}
...
{
  return wait_us > 0 ? delay_ms(dev, (uint32_t)wait_us / 1000U)
                     : VL53LX_ERROR_NONE;
}
