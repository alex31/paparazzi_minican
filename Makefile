.PHONY: all bootloader firmware microcan clean flash flashmicro uartflashmicro
...
MICROCAN_LD := microcan/cfg/STM32G491xE.ld
...
#   make uartflashmicro             # uses DEV=/dev/ttyUSB0 by default
...
all: check-bootloader-size bootloader firmware
	@cp microcan/build_MICROCAN/LAST*.bin /tmp

bootloader:
	$(MAKE) -C bootloader

firmware:
	$(MAKE) -C microcan PLATFORM=MICROCAN $(PROPAGATE_FLAGS) firmware

microcan: firmware

clean:
	@rm -f microcan/cfg/board.h microcan/cfg/.boardgen_*.stamp
	$(MAKE) -C bootloader $(PROPAGATE_FLAGS) clean
	$(MAKE) -C microcan PLATFORM=MICROCAN $(PROPAGATE_FLAGS) clean


check-bootloader-size:
	./set_bootloader_size.pl $(BOOTLOADER_SIZE) $(BOOTLOADER_LD) $(MICROCAN_LD)

flashmicro: flash

flash:
	$(MAKE) -C bootloader $(PROPAGATE_FLAGS) flash
	$(MAKE) -C microcan PLATFORM=MICROCAN $(PROPAGATE_FLAGS) flash
...
uartflashmicro: check-bootloader-size bootloader microcan
	...
	$(call CUBECLI_WRITE,microcan/build_MICROCAN/MICROCAN.elf)
