...
uint32_t pprz_tunnel_Empty_encode(struct pprz_tunnel_Empty* msg, uint8_t* buffer
    ...
    uint32_t bit_ofs = 0;
    memset(buffer, 0, PPRZ_TUNNEL_EMPTY_MAX_SIZE);
    _pprz_tunnel_Empty_encode(buffer, &bit_ofs, msg, 
#if CANARD_ENABLE_TAO_OPTION
    tao
#else
    true
...
#if CANARD_ENABLE_TAO_OPTION
    if (transfer->tao && (transfer->payload_len > PPRZ_TUNNEL_EMPTY_MAX_SIZE)) {
        return true; /* invalid payload length */
    }
#endif
    uint32_t bit_ofs = 0;
    if (_pprz_tunnel_Empty_decode(transfer, &bit_ofs, msg,
#if CANARD_ENABLE_TAO_OPTION
    transfer->tao
#else
    true
