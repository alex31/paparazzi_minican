...
static const int32_t defaultBDTable[VL53LX_TUNING_MAX_TUNABLE_KEY] = {
		TUNING_VERSION,
		TUNING_PROXY_MIN,
...
/* MicroCAN: defaults in flash, mutable tuning in the active device.
 * Lazy initialization preserves Set/GetTuningParameter before DataInit.
 */
_Static_assert(sizeof(((VL53L4CX_Object_t *)0)->TuningData) == sizeof(defaultBDTable),
               "VL53L4CX tuning table size mismatch");

static int32_t *tuningData(VL53LX_DEV Dev)
{
	if (Dev->TuningInitialized == 0U) {
		memcpy(Dev->TuningData, defaultBDTable, sizeof(defaultBDTable));
		Dev->TuningInitialized = 1U;
	}
	return Dev->TuningData;
}

#define BDTable (tuningData(Dev))
