...
int main() {
  ...
  const auto fresh = state.snapshot(start + 1000ms);
  assert(fresh.audioScore.has_value());
  assert(fresh.lightScore.has_value());
  ...
  // and use its positive/zero state for the binary audio presence.
  ...
  const auto beforeExpiry = nominal.snapshot(start + 1700ms);
  assert(beforeExpiry.audioScore.has_value());
  assert(beforeExpiry.lightScore.has_value());
  ...
  const auto heartbeat = nominal.snapshot(start + 3700ms);
  assert(heartbeat.audioScore.has_value());
  assert(heartbeat.lightScore.has_value());
