...
— `snr` part à la fin de chaque bip reconnu ; si le son persiste 200 ms, il part à 5 Hz avec le pic de chaque nouvelle fenêtre. La fin du son envoie la dernière fenêtre partielle. `lit` part une fois par flash lumineux reconnu.
...
— Après 1,5 s sans nouvelle mesure sonore reconnue, `snr` passe à zéro ; `lit` passe à zéro après perte du motif, avec un délai de secours de deux périodes en standard ou 1,8 s en adaptatif.
